...
import { publications, articles } from '@/db/schema'
...
export async function GET(
    ...
    try {
        ...
        if (!pub) {
            return NextResponse.json({ articles: null, pagination: null, error: 'Publication not found' }, { status: 404 })
        }

        const articlesData = await db.query.articles.findMany({
            where: and(
                eq(articles.publicationId, pub.id),
