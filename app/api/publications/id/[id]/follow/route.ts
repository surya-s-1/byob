...
export async function POST(
    ...
    try {
        ...
        if (!pub) {
            return NextResponse.json({ followed: false, error: 'Publication not found' }, { status: 404 })
        }
        ...
        return NextResponse.json({ followed: false, error: 'Internal server error' }, { status: 500 })
    }
}
