...
export async function PUT(
	...
	try {
		...
		if (
			!currentMember ||
			(currentMember.userRole !== 'OWNER' && currentMember.userRole !== 'ADMIN')
		) {
		...
		if (targetMember.userRole === 'OWNER' && currentUser.id !== userId) {
			return NextResponse.json(
				{ updated: false, error: 'Cannot modify OWNER role' },
				{ status: 403 }
			)
		}
		...
		await db
			...
			.set({ userRole: role })
			.where(
				and(eq(publicationMembers.publicationId, id), eq(publicationMembers.userId, userId))
			)
