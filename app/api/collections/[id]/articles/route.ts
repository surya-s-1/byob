	...
	try {
		...
		if (!collection) {
			return NextResponse.json(
				{ articles: null, pagination: null, error: 'Collection not found' },
				{ status: 404 }
			)
		}
		...
		if (collection.visibility === 'PRIVATE' && collection.userId !== currentUser?.id) {
			return NextResponse.json(
				{ articles: null, pagination: null, error: 'Forbidden' },
				{ status: 403 }
			)
		}
		...
		const formattedArticles = collectionArticlesData
			.filter(
				(ca) =>
					ca.article && !ca.article.deletedAt && ca.article.articleStatus === 'PUBLISHED'
			)
			.map((ca) => {
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
	try {
		...
		if (!collection) {
			return NextResponse.json(
				{ added: false, error: 'Collection not found' },
				{ status: 404 }
			)
		}
