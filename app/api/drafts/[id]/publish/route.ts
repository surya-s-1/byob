...
import { articleDrafts, articles, articleAuthors, draftAuthors, publicationMembers } from '@/db/schema'
...
export async function POST(
    ...
    try {
        ...
        if (!draftData.title) {
            return NextResponse.json({ status: null, error: 'Title is required to publish' }, { status: 400 })
        }
