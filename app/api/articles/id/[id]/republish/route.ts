	...
	try {
		...
		if (!article) {
			return NextResponse.json(
				{ republished: false, error: 'Article not found' },
				{ status: 404 }
			)
		}
		...
		if (article.articleStatus !== 'UNPUBLISHED') {
			return NextResponse.json(
				{ republished: false, error: 'Article is not in unpublished status' },
				{ status: 400 }
			)
		}
		...
		if (!visibility || !validVisibilities.includes(visibility)) {
			return NextResponse.json(
				{ republished: false, error: 'Invalid visibility' },
				{ status: 400 }
			)
		}
		...
		return NextResponse.json(
			{ republished: false, error: 'Internal server error' },
			{ status: 500 }
		)
	}
}
