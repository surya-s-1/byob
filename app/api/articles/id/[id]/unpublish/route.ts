...
export async function POST(
    ...
    try {
        ...
        if (!article) {
            return NextResponse.json({ unpublished: false, error: 'Article not found' }, { status: 404 })
        }
        ...
        return NextResponse.json({ unpublished: false, error: 'Internal server error' }, { status: 500 })
    }
}
